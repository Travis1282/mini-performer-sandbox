...
export function useFilterByQueryParams({
  categories,
  regions,
}: useFilterByQueryParamsProps = {}) {
  ...
  const [[fromQueryParam, toQueryParam], setDateQueryParam] =
    useBatchSearchParamState(['from', 'to'])

  const [timeQueryParam, setTimeQueryParam] = useSearchParamState('time')

  const [homeAwayQueryParam, setHomeAwayQueryParam] =
    useSearchParamState('homeAway')

  const [locationQueryParam, setLocationQueryParam] =
    useSearchParamState('location')

  const [categoriesQueryParams, setCategoriesQueryParam] =
    useBatchSearchParamState(['categories'])

  const [typesQueryParams, setTypesQueryParams] = useBatchSearchParamState([
    'types',
  ])
  ...
  function setSelectedCategories(
    ...
    if (typeof action === 'function') {
      ...
      const newParams = newCategories.map(
        (cat: Category) => `${cat.slug}:${cat.id}`
      )
