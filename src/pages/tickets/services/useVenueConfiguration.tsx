...
export const useVenueConfiguration = (
  ...
  const sectionIdToGroup: Record<string, components['schemas']['VenueConfigurationGroup']> =
    useMemo(() => {
      if (!venueConfiguration) {
        return {};
      }
      const { groups, sections } = venueConfiguration;
      const hash: Record<string, components['schemas']['VenueConfigurationGroup']> = {};
...
const VenueConfigurationContext = createContext<undefined | VenueConfigurationContextType>(
  undefined
);
...
export const VenueConfigurationProvider: React.FC<VenueConfigurationProviderProps> = ({
  venueConfig,
  children,
}) => {
 ...
 */
export const useVenueConfigurationContext = (): VenueConfigurationContextType => {
