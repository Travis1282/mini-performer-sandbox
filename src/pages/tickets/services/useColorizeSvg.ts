...
const useColorizeSVG = ({ mapRef }: UseColorizeSvgOptions) => {
  const { venueConfiguration, getColorBySectionId } = useVenueConfigurationContext();
  ...
  const colorSvgSections = useCallback(() => {
    ...
    venueConfiguration?.sections?.forEach((section) => {
      ...
      if (!hasColorized.current) {
        sectionElement.dataset.defaultColor = window.getComputedStyle(sectionElement).fill;
