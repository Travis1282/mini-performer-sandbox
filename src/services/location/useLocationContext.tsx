...
const useLocation = (location: IpLocation & { closestRegionId: string | undefined }) => {
  const [trendingRegion, setTrendingRegion] = useState(location.closestRegionId);

  const changeTrendingRegion = useCallback((regionId: string) => {
...
export const LocationProvider = ({
  ...
  return <LocationContext value={locationWithTrending}>{children}</LocationContext>;
};
