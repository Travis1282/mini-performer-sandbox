...
export function DateRangePicker({
  ...
  const [optionSelected, setOptionSelected] = useState<keyof typeof options | undefined>(
    'any-dates'
  );
  ...
  const selectedRangeText = useMemo(() => {
    const isSameDay = dayjs(selectedDateRange?.from).isSame(selectedDateRange?.to, 'day');
    ...
    return (
      ...
      >
        <div className="z-50 flex h-full flex-col lg:px-2 lg:py-6">
          <div className="px-6">
            <ScrollContainer>
              <div className="no-scrollbar body2 overflow-x-auto rounded-xs py-[6px] font-medium whitespace-nowrap lg:bg-white">
                {Object.keys(options).map((option, index) => (
                  <button
                    className={clsx(
                      'body2 lg:body2 rounded-full px-3 py-1.5 font-medium lg:w-auto lg:px-2.5 lg:py-2',
                    ...
                    key={option}
                    onClick={() => handlePreOptionClick(option as keyof typeof options)}
                  >
          ...
          <DayPicker
            ...
            classNames={{
              caption_end: isMobile ? '!mx-0 rdp-month rdp-caption_end' : 'rdp-caption_end',
              caption_start: isMobile ? 'mx-0! rdp-month rdp-caption_start' : 'rdp-caption_start',
              day: isMobile ? 'w-full h-full !body2' : 'rdp-button_reset rdp-button rdp-day !body2',
              head_cell: 'h6-sm font-normal pl-[5px] pt-[7px] pb-1.5 pr-1',
              month: 'rdp-month h-full font-normal',
              ...
              CaptionLabel: ({ displayMonth }) => {
                return (
                  <div className="w-full text-center">
                    <p className="text-dark body2 font-normal">
                ...
                return (
                  <div className="mb-[9px] flex items-center justify-between">
                    ...
                    <button
                      className={clsx(
                        'text-dark body2 p-2',
                        displayIndex === 0 ? 'visible lg:invisible' : 'invisible',
            ...
            mode="range"
            modifiersClassNames={{
              range_middle: 'text-dark! bg-light! rounded-xs! border-2! border-white! body2',
              range_end: 'text-white! bg-accent! rounded-xs! border-2! border-white! body2',
              range_start: 'text-white! bg-accent! rounded-xs! border-2! border-white! body2',
            }}
            month={defaultMonth}
            numberOfMonths={2}
            onDayClick={() => {
          ...
          />
          <div className="body2 sticky bottom-0 mt-4 flex w-full lg:relative lg:mx-4 lg:w-auto lg:justify-end lg:gap-[45px] lg:px-4">
            <button
              className="lg:text-accent w-1/2 bg-[#3899F8] text-white lg:w-auto lg:rounded-xs lg:bg-transparent"
            ...
            <button
              className={clsx(
                'lg:bg-accent w-1/2 bg-[#196DD2] px-6 py-3 text-white transition-all hover:bg-[#196DD2]/75 lg:w-auto lg:rounded-xs',
...
const ChevronRight = (props: SVGProps<SVGSVGElement>) => (
  <svg fill="none" height={11} width={7} xmlns="http://www.w3.org/2000/svg" {...props}>
    <path
