...
export interface SelectCategoryProps {
  ...
  selected: Category[];
  setSelected: (action: ((prevState: Category[]) => Category[]) | Category[]) => void;
}
            ...
            {children ? (
              ...
              >
                ...
                >
                  {`${
                    selected.length > 0
                      ? `${selected.length > 1 ? `Category: ${selected.length}` : selected[0].name}`
                      : 'Category'
                  }`}
                  {selected.length > 0 && (
                    <span
                      className="flex h-full items-center self-center pr-5 pl-2 text-[8px]!"
                      data-cy="x-button"
