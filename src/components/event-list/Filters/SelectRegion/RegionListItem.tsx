  ...
  return (
    <li
      className={clsx(
        ...
        'hover:bg-accent/10 lg:hover:border-accent relative rounded-none font-medium lg:border-l-4 lg:border-transparent lg:pr-[22px] lg:pl-[22px]'
      )}
      ...
      onClick={onClick}
    >
      {region && search ? makeContainContentBold(`${region.name}`, search) : null}
      {region && !search ? `${region.name}` : null}
      {children}
      <span className="text-accent hidden items-center gap-3 font-semibold group-hover/item:flex">
        Select <ChevronRight />
      </span>
      <span className="block text-gray-400 group-hover/item:hidden">{aditionalInfo}</span>
    </li>
  );
}
...
const ChevronRight = (props: SVGProps<SVGSVGElement>) => (
  <svg fill="none" height={11} width={8} xmlns="http://www.w3.org/2000/svg" {...props}>
    <path
